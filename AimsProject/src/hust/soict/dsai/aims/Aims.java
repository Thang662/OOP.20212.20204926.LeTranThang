			...
			if (menuOption == 1) {
				while (!stopStoreMenu) {
					...
					if (storeMenuOption == 1) {
						...
						tempDVD = Store.searchByTitle(sc.nextLine());
						if (tempDVD == null) {
					...
					// Add a dvd to the cart
						...
						if (tempDVD == null) {
							...
							anOrder.addDigitalVideoDisc(tempDVD);
						}
						Aims.promptEnterKey();
					}
					// Go to cart menu 
			...
			if (menuOption == 2) {
				while (!stopUpdateStore) {
					...
					if (updateStoreOption == 1) {
						...
						Store.addDVD(new DigitalVideoDisc(tempStr1, tempStr2, tempFloat));
						Aims.promptEnterKey();
					}
			...
			if (menuOption == 3) {
				while (!stopCartMenu) {
					...
					// Remove dvd(s) from the cart
						...
						int removedDVDNum = anOrder.removeDigitalVideoDisc(new DigitalVideoDisc(tempTitle));
						if (removedDVDNum != 0) {
							System.out.println("Successfully delete the dvd(s) from the cart");
						}
						else {
							System.out.println("There's is no dvd with that name!");
						}
						Aims.promptEnterKey();
					}
					// Place an order
